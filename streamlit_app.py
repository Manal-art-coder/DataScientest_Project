...
if page == pages[0] : 
  ...
  st.write("Ce projet a été réalisé dans le cadre de notre formation de **Data Analyst** avec l’organisme Datascientest. 
  L’objectif est de prédire le succès d’une campagne marketing en analysant les facteurs influençant la souscription des clients à une offre spécifique. La variable cible étant la souscription ou non à l’offre, nous utilisons l’apprentissage supervisé pour résoudre ce problème de classification binaire. 
  ...
  **Le jeu de données** utilisé provient de la plateforme Kaggle et est accessible à l’adresse suivante : Lien vers le dataset. Ce Streamlit retrace notre démarche, depuis l’exploration et le prétraitement des données jusqu’à la modélisation finale. 
